...
import MPUBG from './MatchesPUBG';
import MHOK from './MatchesHOK';
import MMOB from './MatchesMOB';
import AppTabs from '../../components/AppTabs';
import { getTopImage } from '../../api/imageApi'; 


const MatchesPage = ({ isAuthenticated }) => {
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
          <Box
            sx={{
              width: '100%',
              background: 'radial-gradient(circle at bottom right, rgba(0,37,35,1),  rgba(128,0,128,0.4))',
              borderRadius: 10,
              p: 1,
              display: 'flex',
          ...
          >
            ...
            {topImage && <img src={topImage} alt="Матчи" style={topImageStyles} />}
              <Box
                sx={{
                  position: 'absolute',
                  bottom: 0,
          ...
          >
            <Box className="fade-in" sx={{ px: 2 }}>
              <Routes location={location}>
                <Route path="pubg" element={<MPUBG />} />
                <Route path="hok" element={<MHOK />} />
                <Route path="mob" element={<MMOB />} />
              </Routes>
            </Box>
          </CSSTransition>
        </TransitionGroup>
