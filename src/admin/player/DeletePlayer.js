...
const DeletePlayer = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setSnackbarMessage('Игрок удален успешно!');
      setSnackbarSeverity('success');
      setOpenSnackbar(true);
    } catch (error) {
      setSnackbarMessage('Ошибка при удалении игрока!');
    ...
    <>
      ...
      >
        ...
        <FormControl fullWidth sx={{
          ...
          >
            {players.map((player) => (
              <MenuItem key={player.id} value={player.id} sx={{ fontSize: '1.5rem', '&.Mui-selected': { backgroundColor: 'rgba(255, 255, 255, 0.2)' } }}>
                {player.nickname}
              </MenuItem>
            ))}
          </Select>
